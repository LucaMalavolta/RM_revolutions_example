...
star_dict['inclination'] = 92.0000
star_dict['vsini'] = 4.50 * np.sin(star_dict['inclination']/180.*np.pi)
...
data = np.genfromtxt('../pyorbit_test/full_dataset.dat')

mean_mu, mean_vstar, eclipsed_flux, star_grid = \
    compute_mu_vstar_grid(data[:,0], data[:,6], star_dict, planet_dict, input_ngrid=1001)


import matplotlib.pyplot as plt
plt.figure(figsize=(12,12))

plt.errorbar(mean_mu, data[:,1], yerr=data[:,2], fmt='o', c='k')
plt.errorbar(mean_mu, mean_vstar, fmt='o', c='r')

bjd = np.arange(2454279.3000, 2454279.600, 0.0015)
exptime = np.ones_like(bjd) * 300 # EXPTIME

for i in np.arange(50,150,10):
    star_dict['inclination'] = i
    star_dict['vsini'] = 4.50 * np.sin(star_dict['inclination']/180.*np.pi)
...
for i in np.arange(2.5,6.0,0.5):
    star_dict['vsini'] = i
